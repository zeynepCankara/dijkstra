...
/*
* Increases the distance value of the key
* @param heap Heap datastructure
...
double * dijkstra(struct Graph* graph, int s) {
  int nofVertices = graph->V;
	...
	init_heap(max_heap, nofVertices);
	struct node * newNode = createNode(s);
	newNode->distance = 1.0;
	insert(max_heap, newNode);
	while (max_heap->size > 0)
	{
		struct node * current = extract_max(max_heap);
    ...
    while (temp) {
			if (visited[temp->vertex-1] == 0)
			{
				...
				if (newDist > distance[temp->vertex-1])
				{
					...
					if(contains(max_heap, temp->vertex)){
						increase_key(max_heap, temp->vertex, newDist);
					} else {
